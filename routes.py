...
# AQUI IMPORTAMOS UserForm (Asegúrate de tener el forms.py actualizado)
...
def dashboard():
    ...
    # Datos para gráficos (simples para evitar errores si no hay datos)
    areas = Area.query.all()
    labels_area = [a.nombre for a in areas if len(a.bitacoras) > 0]
    data_area = [len(a.bitacoras) for a in areas if len(a.bitacoras) > 0]
...
# --- GESTIÓN DE USUARIOS (CRUD) - ESTO FALTABA ---
@app.route("/usuarios")
@login_required
@admin_required
def listar_usuarios():
...
def reporte_pdf():
    ...
    col_width = {
        "fecha": 18, "nombre": 38, "km_inicial": 18, "km_final": 18,
        "km_recorridos": 15, "litros": 12, "actividad": 95, 
        "sector": 28, "firma": 22
    }
